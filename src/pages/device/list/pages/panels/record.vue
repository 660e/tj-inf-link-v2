...
export default {
  ...
  mounted() {
    ...
    const p0 = devicesApi.getDevicePropertys(this.id);
    ...
    Promise.all([p0, p1]).then(response => {
      ...
      monitorObject.options = response[0].data;
      monitorObject.value = monitorObject.options.length ? monitorObject.options[0].convertLabel : '';
